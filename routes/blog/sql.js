...
const sql = {
  ...
  update: 'UPDATE blog set title=?, detail=?, summary=?, tag=?, coverImage=? where titleId=?',
  ...
  tagStatistic: 'select tag as name, count(*) as value from blog where tag <> "" group by tag order by count(*) desc',
